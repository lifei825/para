...
def main(ip_file, con_file):
        ...
        for x in open(con_file).readlines():
                ...
                if x.startswith('com:'):
                        ...
                        for i in range(num):        
                                ip, port, user, passwd = iplist[i].split()
                                port = int(port)
                        ...
                        local_file, remote_file = re.findall('up:(.*?)\n', x)[0].split()
                        for i in range(num):        
                                ip, port, user, passwd = iplist[i].split()
                                port = int(port)
                                p = multiprocessing.Process(target=upload, args=(ip, port, user, passwd,
                        ...
                        remote_file, local_file = re.findall('down:(.*?)\n', x)[0].split()
                        for i in range(num):
                                ip, port, user, passwd = iplist[i].split()
                                port = int(port)
                                p = multiprocessing.Process(target=download, args=(ip, port, user, passwd,
                                                                                   local_file, remote_file,))
                                multi.append(p)
...
if __name__ == '__main__':
        direct_run = False
        if os.path.exists('para.log'):
            os.popen('rm -f para.log')
            os.popen('touch para.log')
        # set parameters
        try:
                options, args = getopt(argv[1:], "hyl:c:", ["help", "iplist=", "config="])
        except GetoptError as wr:
                print("Error:", wr)
                print('Plese input: python %s -l iplist -c conf (-y)' % argv[0])
                exit()
...
''')
        ...
        for i in set(iplist)-set(iplog):
            print("\033[1;35;40mERR IP:%s\033[0m" % i)
